...
while (transactions > 0)
{
    ...
    // tillAmount variable created to be used in the conditional breakpoint
    int tillAmount = TillAmount(cashTill);
    Console.WriteLine(TillAmountSummary(cashTill));
    Console.WriteLine($"Expected till value: {registerCheckTillTotal}");
    Console.WriteLine();
}
...
static void MakeChange(int cost, int[] cashTill, int twenties, int tens = 0, int fives = 0, int ones = 0)
{
    ...
    {
        cashTill[3] -= cashTillDiscount[3];
        cashTill[2] -= cashTillDiscount[2];
        cashTill[1] -= cashTillDiscount[1];
        cashTill[0] -= cashTillDiscount[0];
    }
